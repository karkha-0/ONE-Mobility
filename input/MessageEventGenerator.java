...
public class MessageEventGenerator implements EventQueue {
	 ...
	 * @return A random host address from the list
	 */
	protected int drawHostAddress(int hostRange[]) {
		if (hostRange.length == 2 && hostRange[1] > hostRange[0]) {
			// Treat it as a consecutive range of hosts
			return hostRange[0] + rng.nextInt(hostRange[1] - hostRange[0]);
		} else {
			// Treat it as a non-consecutive list of hosts, pick one randomly
			return hostRange[rng.nextInt(hostRange.length)];
		}
	}
	...
	public void updateHostsFromInfectionRouter() {
		...
		if (!infectedNodesSet.isEmpty()) {
			...
			expandedHostRange.addAll(infectedNodesSet);
		
			// Convert the updated list back to an array if necessary
			int[] newHostRange = expandedHostRange.stream().distinct().mapToInt(Integer::intValue).toArray();
